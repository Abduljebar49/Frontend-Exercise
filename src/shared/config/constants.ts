...
export const removeFirstDuplicateClassNames = (classList:string):string =>{
  const classes = classList.split(' ')
  const classIndices: Record<string, number> = {};
  
  classes.forEach((className, index)=>{
    const baseClass = className.split('-')[0]
    classIndices[baseClass] = index;
  })

  const uniqueClasses = Object.keys(classIndices)

  const classesWithOutDuplicates = uniqueClasses.map((baseClass)=> classes[classIndices[baseClass]])

  return classesWithOutDuplicates.join(' ')
}